...
@testset "3. plan over 2 steps" begin
    # Create A problem like run_stlcg-1
...
@testset "4. plan over 2 steps with full spec" begin
    # Create A problem like run_stlcg-1
    ...
    savefig("../images/stlcg-test3.png")

    # Test the curve values are in B2 for enough time
    for traj_ind in range(1,stop=size(x_traj,2))
        x_t = x_traj[:,traj_ind]
        println(A * x_t )
        println(A * x_t - b2 .>= 0.0)
        # println(A * x_t - c1 .>= 0)
        println(pwl_curve1[traj_ind][2])
    end
